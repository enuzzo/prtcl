...
- [x] **Phase 4.13**: Share button — `prtcl.es/create#effect=...&controls=...`. TopBar Share button, hash parse on load to restore state (`src/share/`).
- [x] **Phase 4.15**: Effect WOW pass "Fable" (v0.10.0) — engine buffer fix (`MAX_PARTICLES` 30k → 100k in ParticleSystem, was silently dropping everything past 30k); preset compile safety-net test (`src/test/presets.test.ts` — compiles every preset, checks finite output across times/indices/control extremes, validates `controls:` overrides); full-catalog revamp: Starfield rewritten (6-point warp trails, hyperdrive surge with closed-form travel integral, stellar temperature classes, parallax dust), Nebula Organica rewritten (2-octave domain-warp filaments, 12 embedded star clusters, differential rotation, 5 palettes via `gasPalette`), Fireflies rewritten (Kuramoto-style sync slider, spatial flash waves, Lissajous wander, ground mist, rare blue firefly), Fractal Frequency (5 palettes via `ffPalette`, camera-relative depth shading, crest sparkles — landing `precompiled-frequency.ts` kept in sync), Hopf (fiber comets, sphere drift, 4 palettes via `hopfPalette`), Electromagnetic (charge packets, pole auroras, breathing tilt), Hyperflower (cubed petal definition, 4 palettes via `hfPalette`), Paper Fleet (banking roll from centripetal force), Storm (double-strike lightning flicker), Clifford (7 winding comets), Black Hole (twinkling background stars), Text Wave (diagonal cross-wave + y-bob), Text Dissolve (ember rise), Text Scatter (fixed `palette` → `scatterPalette` dropdown id collision with Axiom); bloom enabled on frequency/hopf/electromagnetic/hyperflower/starfield; 4 new Spirit presets (Ink Ritual, Sodium Ghost, Cherenkov, Acid Pop) + 2 new Flow presets (Aurora Borealis, Sakura Storm); landing: hero/specs/CTA updated to 100k, new § 04 Specimen Index section (static 22-row catalog table, ~1KB gzip), showcase card swap (Nebula → Electromagnetic).
...
Hardcoded at `src/editor/ControlPanel.tsx` near the `addBinding(effectGlobals, 'particleCount', ...)` call. Was 30k (raised to 100k in Phase 4.12). Adjust there when needed. **Must match `MAX_PARTICLES` in `src/engine/ParticleSystem.tsx`** — the geometry buffers are sized once at that constant; counts above it are silently dropped (Float32Array out-of-range writes are no-ops), burning CPU on particles that never render. This bit us when the slider went to 100k while buffers stayed at 30k (fixed in Phase 4.15).
